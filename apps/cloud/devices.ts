...
import type { WriteDevice } from '../../modules/sync/publication/store.js';
import {
  bearer,
  exactObject,
  fail,
  ...
  sha256,
  token,
} from "../../modules/platform/worker/http.js";
...
export async function deviceRoute(
  ...
  if (
    pathname === "/api/v3/device-authorizations" &&
  ...
  if (
    pathname === "/api/v3/device-authorizations/poll" &&
    request.method === "POST"
  ) {
    requireJson(request);
    const body = await readJson(request, 2048);
  ...
  if (
    pathname === "/api/v3/device-authorizations/inspect" &&
    request.method === "POST"
  ) {
    ...
    const resumable=(await env.DB.prepare('SELECT id,name,bound_at,revoked_at,paused,protocol,received_at FROM devices WHERE user_id=? AND history_deleted_at IS NULL ORDER BY bound_at DESC,id LIMIT 1000').bind(user.id).all<{id:string;name:string;bound_at:number;revoked_at:number|null;paused:number;protocol:number;received_at:number|null}>()).results;
    return json({
      deviceName: row.device_name,
      expiresAt: iso(row.expires_at),
      approved: !!row.approved_user_id,
      resumableDevices:resumable.map(d=>({id:d.id,name:d.name,boundAt:iso(d.bound_at),revoked:d.revoked_at!==null,paused:!!d.paused,protocol:d.protocol,receivedAt:d.received_at===null?null:iso(d.received_at)})),
    });
  }
  if (
    pathname === "/api/v3/device-authorizations/approve" &&
    request.method === "POST"
  ) {
    requireSameOrigin(request, env);
    requireJson(request);
      ...
      nonce = crypto.randomUUID();
    {
      ...
      const result = await env.DB.batch([
        ...
        env.DB.prepare(`UPDATE devices SET token_hash=(SELECT token_hash FROM device_authorizations WHERE user_code=? AND approved_user_id=? AND approval_nonce=?),revoked_at=NULL,next_upload_at=0,upload_nonce=NULL,protocol=3
        ...
        env.DB.prepare(`INSERT INTO devices(user_id,id,name,token_hash,bound_at,protocol)
          SELECT approved_user_id,device_id,device_name,token_hash,approved_at,3 FROM device_authorizations
  ...
  if (pathname === "/api/v3/device" && request.method === "DELETE") {
    const device=await deviceAuth(request,env),userId=device.user_id,id=device.id;
    if (id)
      await env.DB.batch([
