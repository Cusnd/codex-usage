...
export function renderCloud(kind:'devices'|'accounts'|'quota',data:CloudSource[]|CloudAccountView[],timezone='Asia/Tokyo'){
  ...
  try {
    ...
    return renderToStaticMarkup(<QueryClientProvider client={client}><MemoryRouter><WebRuntimeProvider runtime={runtime}><Workspace.Provider value={{settings,now:Date.parse('2026-09-12T12:00:00.000Z'),status:undefined}}>{kind==='devices'?<CloudDeviceSettings/>:<CloudAccounts history={kind==='accounts'}/>}</Workspace.Provider></WebRuntimeProvider></MemoryRouter></QueryClientProvider>);
