...
- **可选的云端额度查看。** 用 GitHub 绑定一台主设备后，可在 [quota.esoren.com](https://quota.esoren.com) 查看最后同步的额度与原始采集时间，电脑离线时仍能查看。同步默认关闭，不包含本地用量历史或 Codex 凭据。本分支的同步客户端尚待 npm 发布，使用方式见[云端设置指南](docs/USER_GUIDE.zh-CN.md#在手机或其他电脑查看额度)。
