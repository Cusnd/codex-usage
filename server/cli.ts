...
async function ensureService() {
  ...
  if (current && alive(current.pid)) {
    if (current.port !== port || current.version !== version) throw new Error('Running service port/version differs. Stop it before restarting with this configuration.');
    for (let i = 0; i < 40; i++) {
      try { await control(current); return; }
      catch (error) { if (i === 39) throw error; await delay(250); }
    }
  }
  mkdirSync(dataRoot, { recursive: true });
  const fd = openSync(path.join(dataRoot, 'service.log'), 'a');
  const child = spawn(process.execPath, [path.join(packageRoot, 'dist/server/index.js')], {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    if (record) {
      try { await control(record); if (record.version !== version || record.port !== port) throw new Error('Service configuration mismatch.'); return; }
      catch (error) { if ((error as Error).message === 'Service configuration mismatch.') throw error; }
    }
    await delay(200);
    if (child.exitCode !== null && !instance()) break;
  }
...
async function request(route: string, body?: unknown, method = body === undefined ? 'GET' : 'POST') {
  ...
  if (process.env.CODEX_USAGE_URL) {
    ...
    if (override.protocol !== 'http:' || !['127.0.0.1', 'localhost', '[::1]'].includes(override.hostname) || override.username || override.password || override.pathname !== '/' || override.search || override.hash) throw new Error('CODEX_USAGE_URL must be an HTTP loopback origin.');
    url = override.origin;
  }
...
function skill(action: string) {
  ...
  if (!['install', 'uninstall'].includes(action)) throw new Error('Use skill install|uninstall.');
  if (existsSync(target)) {
    const backup = path.join(root, 'codex-usage-skill-backups', String(Date.now()));
    mkdirSync(path.dirname(backup), { recursive: true }); renameSync(target, backup);
  }
  if (action === 'install') {
