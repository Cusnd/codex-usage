...
预览跳过 GitHub CI 平台矩阵和完整测试套件，保留部署所需的类型检查、前端构建、Worker dry-run、包边界检查和线上 smoke。构建准备可用 `npm run cloud:preview -- --prepare-only`，不会执行远端迁移或部署。云后端与页面必须来自同一构建并同时部署；`--backend-first` 已停用。本地采集器须匹配 `modules/contracts/sync-version.ts` 的当前同步协议、数据格式与解析规则版本，所有未撤销设备完成协议握手后才开放云端统计。源码指纹只用于追踪构建，界面修改和内部优化不要求重装采集器。当前未发布数据结构已合并为 `cloud/migrations/0001_v3.sql`，只初始化空库；旧预览数据库需要重新建立并由当前采集器重采，不保留 v1/v2 迁移链。部署脚本不自动清空数据库。回退时仍须对齐后端、页面、本地端与存储格式。
