...
The web application sends local analytics reads to `POST /api/local/query?route=overview`, with filters in a JSON body. Existing GET routes remain compatible. `overview` returns `{metrics, scope, trend, bucket}`; `scope-summary` returns the first/last observed times and project, projectless-chat and unresolved-chat counts without an identity directory. `bucket: "auto"` selects the all-time trend scale from actual coverage. Omit `from` for all retained history. `project-options` accepts `q`, `limit` and `offset` and returns only a page of `{id, name, kind}` options. POST `filters` supplies model/effort options without a project directory.

Page responses include `meta.projectLabels` for their referenced projects and the selected project filter. Session identities use the source evidence and session title, including explicit projectless-chat icons; a string resembling a session ID is not sufficient evidence. The UI does not assemble a second request containing all returned IDs. The advanced filter loads options when opened, and searches/pages projects on the backend.
...
The current development tree uses protocol v3 for full-history collectors and cloud pages; this is not a claim that production or npm has been upgraded. `CloudSync` in `modules/sync/upload/session.ts` manages binding and the full-history connection. `modules/contracts/sync.ts` defines immutable batches; `modules/sync/protocol/validate-upload.ts` validates the projected-record whitelist. Original titles and paths are included; chat/tool bodies and raw files are excluded. The device bearer stays in local `cloud-credentials.json`, never browser responses or logs. The local backend schedules collection and upload independently of browser tabs. See the [v3 storage, migration and recovery notes](CLOUD_SYNC_V3.md).
...
The local client generates its proposed device token and submits its hash with the device name. A 10-minute binding request has a separate hashed polling secret. Browser approval adds a device without replacing existing devices. D1 stores only device token hashes. Current cloud endpoints use `/api/v3/`; v1/v2 aliases and quota-only collectors have been removed. Device management distinguishes revocation from deletion. Revocation preserves history. Deletion revokes the uploader first, invalidates old read leases, removes that uploader's observations in a durable job, and reconciles events that retain other copies. Mutation batches recheck ownership and revocation so late requests cannot restore deleted history.
...
`modules/usage` supplies record normalization, inherited-prefix handling, canonical candidate selection, execution-origin evidence and metric deltas; `modules/organization` owns project organization. The cloud reconciles all upload observations before filtering by execution device. Upload provenance and execution origin are distinct; conflicting or unproven origins remain explicit. Equal titles, timestamps or token counts alone do not identify one event. Complete source generations publish token facts and task metadata together.

`modules/analytics/query-engine.ts` implements the common query/result and pricing behavior. Tokens are canonical decimal strings or null. Before SQL aggregation, the adapter proves that the filtered row count and maxima keep each intermediate sum within signed 64-bit range; safe sums return text. Otherwise bounded event pages feed exact BigInt aggregation, sorting and costs. Casting an already overflowing sum to text is not a precision strategy. The local adapter executes synchronously; every cloud statement runs against one fixed D1 read cut. Trend buckets use Luxon-generated UTC boundaries, including repeated and skipped DST hours.

The cloud browser uses `POST /api/v3/usage/query?route=local/overview` (or another supported read route), with `lease_id`, timezone and filter parameters in a bounded JSON body. Existing `/api/v3/usage/...` GET routes remain compatible. `local/filters` with `projects:false` omits the project directory. Page rows and their inline labels are resolved within the same guarded D1 read, including its final version/deletion check. Account reads remain independent GET requests. The Workers backend performs grouping, exact aggregation, coverage/counting, identity resolution, search and pagination; the browser renders the returned DTOs.

The cloud data source, local data source and synthetic showcase remain separate adapters sharing the full UI and contracts. Cloud pages use React Query's in-memory result cache, keyed by identity, device scope, filters, timezone and read cut; the retired browser entity mirror is removed. A new cut prepares active results before publishing the replacement page. A new lease for the same cut reuses fixed historical queries; rolling upper time bounds still refresh. Hidden/offline pages stop the cloud usage/account refresh timers. Cloud settings follow the read cut; the shell does not request the legacy local-status DTO or its unused full-history aggregation. Collector and account health use their dedicated cloud APIs. History deletion discards private cached content. Offline pages can retain currently displayed results, while new queries require connectivity.

Confirmed account identity is HMACed with a common per-user account key returned only to authenticated collectors. Unknown identities remain device-scoped unknowns. The v3 account observation endpoint requires `schemaVersion: 3`, a device-wide monotonic sequence and a separate 60-second throttle. Late snapshots cannot switch the current account backwards. Cloud account queries deduplicate confirmed accounts, choose the latest valid quota, merge daily history independently, and preserve measured success values on collection failure. They do not apply the usage-device filter. Collection and receipt times remain distinct.
