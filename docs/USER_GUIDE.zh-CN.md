...
自定义日期按当前时区解释，填好后点击 **应用时间范围**。今天默认按小时，最近七天和三十天默认按日；当前范围内也能手动切换小时、日、周、月粒度。

选择 **全部** 可查看当前设备、项目、模型等筛选范围内的全部已保留记录，页面显示实际记录起点。累计 Token 包含独立会话；项目按逻辑身份计数，同一云端项目跨设备只计一次，独立会话单独计数。这里的总量以已采集或已同步的数据为准。全历史趋势随跨度自动按日、周或月显示，不提供上一时段比较；下钻后可返回原来的全部时间范围。

独立会话使用 Session 标题，缺少标题时显示“会话 + 短 ID”；旁边的对话气泡图标悬停显示 **Projectless chat**。Git 项目、App 项目和归属待识别记录使用各自的图标，独立会话不再显示为“未命名项目”。
...
云端页面采用完全在线模式，按需读取统计和明细，只在当前浏览器会话的内存中复用结果，不再先下载近期历史、再补齐完整副本，也不持久保存统计用于离线查看。同一组用量结果和项目名称使用同一个已发布读取版本。云端 **刷新全部** 取得云端已经应用的新读取结果，不会命令采集电脑扫描或上传；设备覆盖状态继续描述采集端进度，不再表示浏览器下载进度。

本机采集间隔由各设备本地设置控制；云端用量／额度刷新间隔、时区、费用开关和模型价格保存在当前 GitHub 用户下。用量间隔为 0 时关闭周期更新，手动刷新仍可用。网络失败时，当前已显示的结果可以暂留，并提示离线或读取错误；重新加载或重新打开需要联网，其他页面和筛选不保证离线可用。退出登录或删除历史使相关会话结果失效。[完全在线浏览器设计](design/cloud-browser-simplification-2026-09-13.md)替代此前的近期／全量浏览器缓存行为。
...
云端“跟随系统时区”在每个访问浏览器中分别解析当地时区，并在重新回到窗口和每分钟检查变化；手动时区由所有访问端共用。账户额度按当前用户在线读取，独立于用量设备筛选，并保留自身采集时点；用量查询只在会话内存中复用，仍要求相同页面、筛选、时区和读取版本。
