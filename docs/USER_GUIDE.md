...
The cloud page is fully online. It reads the requested statistics and details from the server and keeps results only in the current browser session's memory; it does not download a recent/full history copy or save statistics for offline use. Related usage results and project labels share one published read version. **刷新全部** obtains a new read of applied cloud results; it does not ask collecting computers to scan or upload. Device coverage continues to describe collection and upload progress, rather than a browser download.

Each computer controls its own collection interval, while cloud usage/account refresh intervals, timezone and prices are saved for the GitHub user. A zero usage interval disables periodic updates while retaining manual refresh. On a network failure, already displayed results may remain with an offline/error notice. Reloading or reopening needs a connection, and other pages or filters have no offline availability guarantee. Logging out or invalidating deleted history discards the affected session results. The [online browser design](design/cloud-browser-simplification-2026-09-13.md) replaces the earlier recent/full browser-cache behavior.
...
Cloud system-timezone mode resolves the timezone separately in each browser and checks it on window focus and once per minute. Manual mode uses the shared saved timezone. Account observations are read online for the current user independently of usage-device filters and retain their own collection times. Usage-query reuse is confined to session memory and the matching page, filters, timezone and read version.
