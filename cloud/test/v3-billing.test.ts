...
it('replays native mode evidence atomically, preserves old cuts, and switches subscription/API billing without changing tokens',async()=>{
  ...
  const call=(route:string,method='GET',body?:unknown)=>worker.fetch(new Request(origin+route,{method,
  ...
  const send=async(batch:UploadBatch)=>{
    const response=await worker.fetch(new Request(origin+'/api/v3/ingest',{method:'POST',headers:{Authorization:'Bearer '+credential,'Content-Type':V3_CONTENT_TYPE},body:gzipSync(stableJson(batch))}),env);
  ...
  for(const batch of fixture.old)await send(batch as UploadBatch);
  expect((await call('/api/v3/settings','PATCH',{costEnabled:true})).status).toBe(200);
  const old=await pin(),before=await summary(old.lease_id);
  expect(old.settings.officialApiPricing).toBe(false);
  expect(before).toMatchObject({totalTokens:'3300',cost:{currency:'USD',billingBasis:'subscription',amount:null,complete:false}});
  const eventIds=(await env.DB.prepare('SELECT event_id FROM v3_events WHERE user_id=? ORDER BY event_id').bind(user).all<{event_id:string}>()).results.map(r=>r.event_id);
  for(const [index,batch] of fixture.upgraded.entries()){
    await send(batch as UploadBatch);const current=await summary();expect(current.totalTokens).toBe('3300');
    if(index<fixture.upgraded.length-1)expect(current.cost.amount).toBeNull();
  }
  const after=await summary();expect(after).toEqual(fixture.expected.subscription);
  expect((await env.DB.prepare('SELECT event_id FROM v3_events WHERE user_id=? AND epoch=(SELECT active_epoch FROM v3_sync_domains WHERE user_id=?) ORDER BY event_id').bind(user,user).all<{event_id:string}>()).results.map(r=>r.event_id)).toEqual(eventIds);
  expect(await summary(old.lease_id)).toEqual(before);
  ...
  await send(fixture.old[0] as UploadBatch);expect(await summary()).toEqual(fixture.expected.api);
