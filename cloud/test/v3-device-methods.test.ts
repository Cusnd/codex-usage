...
async function fixture(){
  ...
  const uploaded=await worker.fetch(new Request(origin+'/api/v3/ingest',{method:'POST',headers:{Authorization:'Bearer '+credential,'Content-Type':V3_CONTENT_TYPE},body:gzipSync(stableJson(batch))}),env);
  ...
  const summary=async()=>{const response=await worker.fetch(new Request(origin+'/api/v3/usage/local/summary',{headers:{Cookie:SESSION_COOKIE+'='+session}}),env);expect(response.status).toBe(200);return (await response.json<{data:{totalTokens:string}}>()).data.totalTokens;};
...
it('keeps normal pause, revoke and history deletion as separate authenticated actions',async()=>{
  ...
  expect(await f.state()).toMatchObject({paused:0,revoked_at:expect.any(Number),history_deleted_at:null});expect(await f.summary()).toBe('20');
  expect([200,202]).toContain((await f.call('/history','DELETE')).status);await f.drain();
  expect(await f.state()).toMatchObject({revoked_at:expect.any(Number),history_deleted_at:expect.any(Number)});expect(await f.summary()).toBe('0');
});
