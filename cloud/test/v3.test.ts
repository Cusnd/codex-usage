...
import { domain,guard,endGuard,currentDevice,receive,receipt,entityStatements } from '../src/v3/store';
import { metricStatements } from '../src/v3/metrics';
import { cleanupVersions,entities as readEntities,manifest as readManifest } from '../src/v3/snapshots';
...
async function actor(user?:string):Promise<Actor>{const id=user||crypto.randomUUID(),session=token(),device=crypto.randomUUID(),credential=token();if(!user)await env.DB.prepare('INSERT INTO users(id,github_id,login,created_at) VALUES(?,?,?,?)').bind(id,id,'v3-test',Date.now()).run();await env.DB.prepare('INSERT INTO sessions(token_hash,user_id,expires_at) VALUES(?,?,?)').bind(await sha256(session),id,Date.now()+86400000).run();await env.DB.prepare('INSERT INTO devices(id,user_id,name,token_hash,bound_at) VALUES(?,?,?,?,?)').bind(device,id,device,await sha256(credential),Date.now()).run();return {user:id,session,device,credential,collector:crypto.randomUUID()};}
async function batch(a:Actor,options:{seq?:number;generation?:number;from?:number;complete?:boolean;response?:string;tokens?:string;originDevice?:string;originKind?:'preserved'|'execution';source?:string;record?:any;timestamp?:string}={}):Promise<UploadBatch>{
  ...
  return {protocol:3,schema_version:1,extractor_version:1,collector_id:a.collector,producer_epoch:'epoch',lane:'live',lane_seq:seq,batch_id:crypto.randomUUID(),records_hash:await sha256(stableJson(records)),sources:[{source_id:source,generation,kind:'session',from_cursor:from,to_cursor:from+100,snapshot_eof:from+100,context_hash:await sha256(stableJson(context)),context,replace_start:from===0,replace_end:options.complete!==false,generation_complete:options.complete!==false,available:true,trailing_bytes:0}],records,metadata:[]};
}
async function call(a:Actor,path:string,method='GET',body?:unknown){return worker.fetch(new Request(origin+path,{method,headers:{Origin:origin,Cookie:SESSION_COOKIE+'='+a.session,...body===undefined?{}:{'Content-Type':'application/json'}},body:body===undefined?undefined:JSON.stringify(body)}),env);}
async function upload(a:Actor,b:UploadBatch){const wire=gzipSync(stableJson(b));return worker.fetch(new Request(origin+'/api/v3/ingest',{method:'POST',headers:{Authorization:'Bearer '+a.credential,'Content-Type':V3_CONTENT_TYPE},body:wire}),env);}
...
async function legacy(a:Actor,tokens:string,revision=1,response:string|null=null){
  const thread={id:'thread',title:'Old history',titleUpdatedAt:null,project:'/old',source:'cli',parentId:null,subagentParentId:null,forkedFromId:null},manifest={schemaVersion:2,datasetId:'old-dataset',thread,revision,parserVersion:1,collectedAt:new Date().toISOString(),eventCount:1,chunkCount:1,contentHash:'0'.repeat(64)};
  await env.DB.batch([env.DB.prepare('INSERT INTO usage_revisions(user_id,device_id,dataset_id,thread_id,revision,parser_version,collected_at,manifest,received_at,committed) VALUES(?,?,?,?,?,1,?,?,?,1)').bind(a.user,a.device,'old-dataset','thread',revision,manifest.collectedAt,JSON.stringify(manifest),Date.now()),env.DB.prepare("INSERT INTO usage_records(user_id,device_id,dataset_id,thread_id,revision,event_key,turn_id,response_id,at,project,model,effort,kind,incomplete,input_tokens,output_tokens,total_tokens) VALUES(?,?,?,?,?,'old-event','turn',?,?, '/old','gpt-5',NULL,'record',0,?,0,?)").bind(a.user,a.device,'old-dataset','thread',revision,response,new Date().toISOString(),tokens,tokens),env.DB.prepare('INSERT INTO usage_heads(user_id,device_id,dataset_id,thread_id,revision) VALUES(?,?,?,?,?) ON CONFLICT(device_id,dataset_id,thread_id) DO UPDATE SET revision=excluded.revision').bind(a.user,a.device,'old-dataset','thread',revision)]);
}
...
async function authorization(a:Actor){const credential=token(),response=await call(a,'/api/v2/device-authorizations','POST',{deviceName:'Recovered computer',tokenHash:await sha256(credential)});expect(response.status).toBe(201);return {...await response.json<any>(),credential};}
const approve=(a:Actor,p:{userCode:string},target:string|null)=>call(a,'/api/v2/device-authorizations/approve','POST',{code:p.userCode,replaceDeviceId:target});
const poll=(a:Actor,p:{requestId:string;pollSecret:string})=>call(a,'/api/v2/device-authorizations/poll','POST',{requestId:p.requestId,pollSecret:p.pollSecret});
...
it('resumes an owned device with the same history and ID, rotates its upload identity, and replays approval without another rotation',async()=>{
  const a=await actor(),original=await batch(a);await upload(a,original);const old=await currentDevice(env.DB,a.user,a.device),head=await domain(env.DB,a.user),bound=await env.DB.prepare('SELECT bound_at FROM devices WHERE id=?').bind(a.device).first('bound_at'),p=await authorization(a),inspect=await (await call(a,'/api/v2/device-authorizations/inspect','POST',{code:p.userCode})).json<any>();
...
it('migrates only committed v2 heads exactly, preserves unknown origin, and retires legacy facts only after explicit complete-source handoff',async()=>{
  const a=await actor();await legacy(a,'9223372036854775807');await domain(env.DB,a.user);expect((await call(a,'/api/v3/sync/read','POST',{scope:'full'})).status).toBe(409);await drain(a);const imported=await events(a);expect(imported).toHaveLength(1);expect(imported[0]).toMatchObject({total_tokens:'9223372036854775807',origin_device_id:null,identity_quality:'source_position'});
  const summary=await (await call(a,'/api/v3/usage/local/summary')).json<any>();expect(summary.data.totalTokens).toBe('9223372036854775807');
  const fresh=await batch(a,{tokens:'100'}),prepare=await worker.fetch(new Request(origin+'/api/v3/legacy/prepare',{method:'POST',headers:{Authorization:'Bearer '+a.credential,'Content-Type':'application/json'},body:JSON.stringify({collector_id:a.collector,replacements:[{type:'legacy_replacement',dataset_id:'old-dataset',thread_id:'thread',sources:[{source_id:'source',generation:1}]}]})}),env);expect(prepare.status).toBe(200);expect((await upload(a,fresh)).status).toBe(200);expect((await events(a)).map(e=>e.total_tokens)).toEqual(['9223372036854775807']);
  const handoff={...await batch(a,{seq:2}),sources:[],records:[],metadata:[{type:'legacy_replacement' as const,dataset_id:'old-dataset',thread_id:'thread',sources:[{source_id:'source',generation:1}]}]};handoff.records_hash=await sha256(stableJson([]));
  expect((await upload(a,handoff)).status).toBe(202);await drain(a);expect((await events(a)).map(e=>e.total_tokens)).toEqual(['100']);expect(await env.DB.prepare('SELECT retired_revision FROM v3_legacy_heads WHERE user_id=?').bind(a.user).first('retired_revision')).toBe(1);
  await legacy(a,'45',2);expect((await domain(env.DB,a.user)).legacy_baseline_pending).toBe(1);await drain(a);expect((await events(a)).map(e=>e.total_tokens).sort()).toEqual(['100','45']);
  const second={...handoff,lane_seq:3,batch_id:crypto.randomUUID()};expect((await upload(a,second)).status).toBe(202);await drain(a);expect((await events(a)).map(e=>e.total_tokens)).toEqual(['100']);
},30000);

it('keeps legacy data when replacement source is unavailable and uses strong response identity across v2/v3 copies',async()=>{
  const a=await actor();await legacy(a,'100',1,'shared-response');await domain(env.DB,a.user);await drain(a);await upload(a,await batch(a,{response:'shared-response'}));expect(await events(a)).toHaveLength(1);expect((await events(a))[0].origin_device_id).toBe(a.device);
  const b=await batch(a,{seq:2});b.sources=[];b.records=[];b.metadata=[{type:'legacy_replacement',dataset_id:'old-dataset',thread_id:'thread',sources:[{source_id:'missing',generation:1}]}];b.records_hash=await sha256(stableJson([]));expect((await upload(a,b)).status).toBe(409);expect(await events(a)).toHaveLength(1);expect(await env.DB.prepare('SELECT retired_revision FROM v3_legacy_heads WHERE user_id=?').bind(a.user).first('retired_revision')).toBe(0);
});
...
it('reports applied device coverage separately from initial inventory and freezes settings defaults at the read cut',async()=>{
  ...
  const status=await worker.fetch(new Request(origin+'/api/v3/sync/status',{method:'PUT',headers:{Authorization:'Bearer '+a.credential,'Content-Type':'application/json'},body:JSON.stringify({collectedAt:new Date().toISOString(),totalThreads:1,initialComplete:true,error:null})}),env);expect(status.status).toBe(200);expect((await device()).initialComplete).toBe(true);
...
it('discards a paused rebuild without publishing pending facts, resumes it, and keeps rejected revocation writes out',async()=>{
  ...
  // Explicit handoff also rebuilds through the same device authorization fence.
  await legacy(a,'50');await drain(a);const handoff={...await batch(a,{seq:4}),sources:[],records:[],metadata:[{type:'legacy_replacement' as const,dataset_id:'old-dataset',thread_id:'thread',sources:[{source_id:'source',generation:2}]}],records_hash:await sha256(stableJson([]))};expect((await upload(a,handoff)).status).toBe(202);await env.DB.prepare('UPDATE devices SET revoked_at=? WHERE id=?').bind(Date.now(),a.device).run();await drain(a);expect((await receipt(env.DB,a.user,handoff.batch_id))?.status).toBe('cancelled');expect((await events(a)).map(e=>e.total_tokens).sort()).toEqual(['50','8']);expect((await domain(env.DB,a.user)).mode).toBe('ready');
},30000);
...
it('restarts an interrupted legacy baseline without exposing the deleting domain or losing the remaining uploader history',async()=>{
  const a=await actor(),b=await actor(a.user);await upload(a,await batch(a));await legacy(b,'50');await advanceJobs(env.DB,{user:b.user,job_id:'legacy-migrate',maxSteps:4,budgetMs:10000});expect((await domain(env.DB,b.user)).mode).toBe('rebuilding');
  await env.DB.prepare('UPDATE devices SET revoked_at=?,history_deleted_at=? WHERE user_id=? AND id=?').bind(Date.now(),Date.now(),a.user,a.device).run();await advanceJobs(env.DB,{user:b.user,job_id:'legacy-migrate',maxSteps:1});expect((await domain(env.DB,b.user)).mode).toBe('deleting');expect(await env.DB.prepare("SELECT payload,checkpoint FROM v3_jobs WHERE user_id=? AND job_id='legacy-migrate'").bind(b.user).first()).toEqual({payload:'{}',checkpoint:'{}'});
  expect((await call(a,'/api/v3/sync/read','POST',{scope:'full'})).status).toBe(409);expect((await call(a,'/api/v3/devices/'+a.device+'/history','DELETE')).status).toBe(200);await drain(b);expect((await events(b)).map(e=>e.total_tokens)).toEqual(['50']);expect(await domain(env.DB,b.user)).toMatchObject({mode:'ready',rebuild_job:null,legacy_baseline_pending:0});
});
...
it('separates received/applied watermarks across gaps and resumes a persisted request without the original HTTP call',async()=>{
  ...
  const receipts=await worker.fetch(new Request(origin+'/api/v3/receipts?ids='+second.batch_id,{headers:{Authorization:'Bearer '+a.credential}}),env),data=await receipts.json<any>();expect(data.receipts[0]).toMatchObject({contiguous_received_seq:2,contiguous_applied_seq:2});expect(await events(a)).toHaveLength(2);
});
it('pins paginated entity bodies and relations to a cut, then invalidates old reads after deletion',async()=>{
  ...
  await call(a,'/api/v3/devices/'+a.device+'/history','DELETE');expect((await call(a,`/api/v3/sync/read/${read.lease_id}/manifest`)).status).toBe(409);
});
