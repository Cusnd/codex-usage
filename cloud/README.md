...
`shared/cloud.ts` and `shared/cloud-accounts.ts` define current account observations with `schemaVersion: 3`. All cloud API endpoints use `/api/v3/`; v1/v2 aliases and quota-only collectors have been removed. Chat text, tool bodies, raw identities and credentials are excluded. API ownership comes from authenticated sessions/devices. Private responses use `Cache-Control: no-store`.
