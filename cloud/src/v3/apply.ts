...
async function verifyOrigin(db:D1Database,user:string,device:string,observation:Observation):Promise<Observation> {
  ...
  if(!await db.prepare('SELECT id FROM devices WHERE user_id=? AND id=?').bind(user,origin.device_id).first())fail(400,'INVALID_ORIGIN_EVIDENCE','执行来源不属于当前空间。');
...
export async function prepareMaterialization(db:D1Database,h:Domain,initial:CandidateRow[],mutations:Map<string,CandidateRow|null>,additionalIds:string[]=[],preserveRemovedOrigins=false):Promise<{statements:D1PreparedStatement[];changes:EntityMutation[];deltas:EventDelta[]}> {
  ...
  for(const id of ids)for(const updated of reconcileOriginEvidence([...current.values()].filter(r=>r.event_id===id),before.filter(r=>r.event_id===id),preserveRemovedOrigins)){
  ...
  const eligible=reconcileTurns(visible).filter(c=>c.kind==='record'||c.turn_id===null||!externalTurns.has(pair(c))),canonical=new Map<string,CanonicalEvent|null>();for(const id of ids){const allowed=new Set(eligible.filter(c=>c.event_id===id).map(c=>c.observation_id));canonical.set(id,await canonicalFromRows([...current.values()].filter(r=>allowed.has(r.observation_id))));}
...
export async function materializeCanonical(db:D1Database,h:Domain,canonical:Map<string,CanonicalEvent|null>,overrides?:Map<string,UserAssignment|null>):Promise<{statements:D1PreparedStatement[];changes:EntityMutation[];deltas:EventDelta[]}> {
  ...
  const rules=await assignmentRules(db,h.user_id,ids);for(const id of ids)canonical.set(id,withAssignment(canonical.get(id)!,overrides?.has(id)?overrides.get(id)!:rules.get(id)??null));
  for(const group of chunks([...ids])){const old=await db.prepare('SELECT entity_id,payload,revision FROM v3_entity_versions WHERE user_id=? AND epoch=? AND kind=\'event\' AND valid_to IS NULL AND entity_id IN(SELECT value FROM json_each(?))').bind(h.user_id,h.active_epoch,stableJson(group)).all<{entity_id:string;payload:string;revision:number}>();for(const row of old.results)oldEvents.set(row.entity_id,row);}
  ...
  for(let attempt=0;attempt<3;attempt++){
    ...
    for(const source of b.sources){
      ...
      for(const original of records){
        const contentHash=await sha256(stableJson(original)),old=priorObservations.find(r=>r.observation_id===original.observation_id);
        ...
        const row=await verifyOrigin(db,user,device.id,original),beforeState=state,parsed=consumeProjected(state,row,dep);state=parsed.state;
    ...
    if(!publishEpoch)for(const old of replacements){const related=await db.prepare('SELECT * FROM v3_candidates WHERE user_id=? AND collector_id=? AND source_id=?').bind(user,b.collector_id,old.source_id).all<CandidateRow>();for(const r of related.results){initial.push(r);if(r.generation===old.generation)candidateMutations.set(r.observation_id,{...r,active:0});else if(r.generation===sourceUpdates.find(s=>s.source_id===old.source_id)!.generation&&!candidateMutations.has(r.observation_id))candidateMutations.set(r.observation_id,{...r,active:1});}}
    // A replacement's final packet may contain no metadata; promote the metadata staged earlier too.
    if(!publishEpoch)for(const old of replacements){const generation=sourceUpdates.find(s=>s.source_id===old.source_id)!.generation;const staged=(await db.prepare('SELECT source_id,generation,thread_id,payload FROM v3_source_threads WHERE user_id=? AND collector_id=? AND source_id=? AND generation=?').bind(user,b.collector_id,old.source_id,generation).all<{source_id:string;generation:number;thread_id:string;payload:string}>()).results;for(const t of staged)if(!threadWrites.some(w=>w.source_id===t.source_id&&w.generation===t.generation&&w.thread_id===t.thread_id))threadWrites.push(t);}
    ...
    if(!publishEpoch)for(const id of new Set(threadWrites.map(t=>t.thread_id))){
      const old=await db.prepare('SELECT payload,revision FROM v3_threads WHERE user_id=? AND thread_id=?').bind(user,id).first<{payload:string;revision:number}>();
      ...
      if(value&&stableJson(value)!==old?.payload){const revision=(old?.revision||0)+1,payload=stableJson(value);threadStatements.push(db.prepare('INSERT INTO v3_threads(user_id,thread_id,payload,revision) VALUES(?,?,?,?) ON CONFLICT(user_id,thread_id) DO UPDATE SET payload=excluded.payload,revision=excluded.revision').bind(user,id,payload,revision));changes.push({kind:'thread',id,revision,value,thread_id:id});}
    }
    const op=crypto.randomUUID(),statements:D1PreparedStatement[]=[guard(db,h,op,device,job)];
    if(publishEpoch)for(const s of sourceUpdates)statements.push(db.prepare('UPDATE v3_candidates SET active=? WHERE user_id=? AND collector_id=? AND source_id=? AND generation=?').bind(s.active,user,b.collector_id,s.source_id,s.generation));
    for(const m of b.metadata)if(m.type==='source_availability')statements.push(db.prepare('UPDATE v3_sources SET available=? WHERE user_id=? AND collector_id=? AND source_id=?').bind(Number(m.available),user,b.collector_id,m.source_id));
    for(const old of replacements)statements.push(db.prepare('UPDATE v3_sources SET active=0 WHERE user_id=? AND collector_id=? AND source_id=? AND active=1').bind(user,b.collector_id,old.source_id));
    ...
    for(const source of sourceUpdates)if(!JSON.parse(source.legacy_state!).deferred)statements.push(db.prepare('DELETE FROM v3_deferred_records WHERE user_id=? AND collector_id=? AND source_id=? AND generation=?').bind(user,b.collector_id,source.source_id,source.generation));
    for(const s of b.sources)if(s.generation_complete){statements.push(db.prepare(`INSERT INTO v3_jobs(user_id,job_id,kind,payload,created_at,updated_at)
      SELECT d.user_id,'dependency:'||d.collector_id||':'||d.source_id||':'||d.generation,'dependency',json_object('collector_id',d.collector_id,'source_id',d.source_id,'generation',d.generation),?,? FROM v3_dependencies d WHERE d.user_id=? AND d.parent_thread_id=?
      ON CONFLICT(user_id,job_id) DO UPDATE SET state='pending',next_attempt_at=0 WHERE v3_jobs.state<>'running'`).bind(Date.now(),Date.now(),user,s.context.thread_id));}
    // Superseded generations are no longer current facts or pending replay inputs.
    for(const old of replacements){const generation=sourceUpdates.find(s=>s.source_id===old.source_id)!.generation;for(const table of ['v3_candidates','v3_observations','v3_source_threads','v3_deferred_records','v3_dependencies','v3_sources'])statements.push(db.prepare(`DELETE FROM ${table} WHERE user_id=? AND collector_id=? AND source_id=? AND generation<?`).bind(user,b.collector_id,old.source_id,generation));}
