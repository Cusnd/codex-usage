...
export async function advanceJobs(db:D1Database,options:{user?:string;job_id?:string;maxSteps?:number;budgetMs?:number;knownBatch?:UploadBatch;timing?:SyncTiming}={}) {
  const deadline=Date.now()+(options.budgetMs??5000);let processed=0;
  while(processed<(options.maxSteps??4)&&Date.now()<deadline){const job=await measured(options.timing,'job_claim',()=>claimJob(db,options.user,options.job_id));if(!job)break;processed++;
    try {
      let done:boolean;
      if(job.kind==='apply'){const id=JSON.parse(job.payload).batch_id,known=options.knownBatch;let b:UploadBatch;if(options.user===job.user_id&&known&&known.batch_id===id)b=known;else{const row=await measured(options.timing,'pending_read',()=>db.prepare('SELECT wire FROM v3_pending_inputs WHERE user_id=? AND batch_id=?').bind(job.user_id,id).first<{wire:ArrayBuffer}>());if(!row){await failJob(db,job,'MISSING_PENDING_INPUT');continue;}b=await decodeWire(new Uint8Array(row.wire));}done=await applyBatch(db,job.user_id,b,job,undefined,options.timing);}
      ...
      if(error instanceof HttpError){if(job.kind==='legacy_stage'&&['DEVICE_PAUSED','DEVICE_REVOKED','REBUILD_SUPERSEDED'].includes(error.code))await cancelLegacyStage(db,job);else if(error.code==='DEVICE_PAUSED'){if(job.kind==='rebuild')await restartRebuild(db,job);else await release(db,job,30_000,error.code);}else if(error.code==='REBUILD_SUPERSEDED'&&job.kind==='rebuild')await recoverSupersededRebuild(db,job);else await failJob(db,job,error.code,error.code==='DEVICE_REVOKED');}
      else if(isCasFailure(error))await release(db,job,250,'WRITE_CONFLICT');
      else {await release(db,job,Math.min(60_000,1000*2**Math.min(job.attempts,6)),'APPLY_FAILED');throw error;}
    }
  }
