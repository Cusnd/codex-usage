...
export async function createRead(db:D1Database,user:string,scope:'recent'|'full',devices:string[]=[],fromLeaseId?:string) {
  await assertDeviceScope(db,user,devices);const h=await domain(db,user);if(h.mode==='deleting')fail(409,'HISTORY_UPDATING','历史删除正在处理。');if(!fromLeaseId&&(h.mode==='rebuilding'||h.legacy_baseline_pending))fail(409,'DATASET_UPDATING','历史基线正在后台构建，请继续使用已有固定版本并稍后重试。');
  ...
  if(fromLeaseId){await getRead(db,user,fromLeaseId);await db.prepare(`INSERT INTO v3_read_leases(user_id,lease_id,epoch,cut,deletion_version,organization_version,config_version,scope,from_at,device_ids,created_at,expires_at,max_expires_at,coverage,settings)
    ...
    SELECT user_id,?,active_epoch,commit_seq,deletion_version,organization_version,config_version,?,?,?,?,?,?,json_object('sources',json((SELECT COALESCE(json_group_array(json_object('device_id',c.device_id,'collector_id',s.collector_id,'source_id',s.source_id,'generation',s.generation,'cursor',s.cursor,'snapshot_eof',s.snapshot_eof,'complete',s.complete,'active',s.active,'available',s.available,'trailing_bytes',s.trailing_bytes)),'[]') FROM v3_sources s JOIN v3_collectors c USING(user_id,collector_id) WHERE s.user_id=h.user_id))),json_patch(?,COALESCE((SELECT payload FROM v3_settings WHERE user_id=h.user_id),(SELECT settings FROM users WHERE id=h.user_id),'{}')) FROM v3_sync_domains h WHERE user_id=? AND mode='ready' AND legacy_baseline_pending=0`).bind(id,scope,scope==='recent'?new Date(now-30*86400_000).toISOString():null,stableJson(devices),now,now+15*MINUTE,now+60*MINUTE,stableJson(defaultSettings),user).run();
...
export async function changes(db:D1Database,user:string,epoch:string,after:number,limit=20,leaseId?:string):Promise<ChangesPage> {
  ...
  const h=await domain(db,user);if(h.active_epoch!==epoch||after<h.changes_floor||after>h.commit_seq||h.mode==='deleting')fail(409,'BASELINE_REQUIRED','需要新的完整同步基线。');if(h.mode==='rebuilding'||h.legacy_baseline_pending)fail(409,'DATASET_UPDATING','历史基线正在后台构建。');
