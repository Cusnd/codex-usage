...
export async function createRead(db:D1Database,user:string,scope:'recent'|'full',devices:string[]=[],fromLeaseId?:string) {
  await assertDeviceScope(db,user,devices);const h=await domain(db,user);if(h.mode==='deleting')fail(409,'HISTORY_UPDATING','历史删除正在处理。');if(!fromLeaseId&&(h.mode==='rebuilding'||h.legacy_baseline_pending))fail(409,'DATASET_UPDATING','完整历史正在后台准备，完成后将自动显示。');
