...
Windows 上的本地 Codex 用量分析工具：账户额度和每日用量、本地项目／模型／推理强度分析、任务与全局轮次明细、时段对比、可选 API 参考成本，以及 CLI + Web + Skill。

**Windows x64 · Node.js >=26.7.0 · Apache-2.0**

## 界面预览

以下截图使用独立生成的示例数据，不包含真实账户或会话。

![示例用量分析](docs/images/analysis.png)

![启动与访问设置](docs/images/settings.png)

## 安装与使用

将 [Agent 安装文档](https://github.com/Cusnd/codex-usage/blob/main/docs/INSTALL_FOR_AGENTS.md) 交给 Agent，并告诉它“请按照文档帮我安装，并说明如何使用”。配套安装脚本会校验下载文件、复用兼容 Node 或安装用户级 Node，并配置用户 PATH。
...
```powershell
npm install --global https://github.com/Cusnd/codex-usage/releases/download/v0.1.1/codex-detailed-usage-0.1.1.tgz
codex-usage
```

安装的是本项目的 GitHub Release 构建包，不是 npm 上名为 `codex-usage` 的其他包。前端已预构建，不要求下载源码或编译；安装时需要联网下载依赖。

```powershell
codex-usage                                  # 启动或复用服务，并打开 Web
codex-usage start                            # 仅后台启动
codex-usage status --json                    # 查看状态，不自动启动
codex-usage summary --days 7 --json           # 查询本地用量，必要时启动服务
codex-usage breakdown --days 7 --group-by model --json
codex-usage agents --id TASK_ID --json        # 当前 agent、后代与团队
codex-usage refresh --source local --wait --json
codex-usage skill install                    # 安装 Codex Skill
codex-usage doctor --json                    # 诊断运行环境
codex-usage stop                             # 正常关闭服务
```

直接运行 `codex-usage` 不需要输入 `open`。关闭网页或启动终端不会关闭后台服务。设置页可以开启“登录 Windows 后后台启动”，也可使用 `codex-usage autostart enable`、`disable` 或 `status`。默认关闭；开启后只启动服务，不弹出浏览器。禁用影响未来登录，不停止当前进程。

服务运行后，输入 [usage.esoren.com](https://usage.esoren.com) 即可跳转到 **当前设备** 的 `http://127.0.0.1:8765/`，地址栏随后显示本地地址。域名不能启动或安装程序，需要联网，不提供远程电脑访问。自定义端口须使用对应本地地址，域名仍指向 8765。离线时可用 CLI 或本地地址查看本地统计。

## 数据目录、升级和卸载

缓存、日志和实例信息默认位于 `%LOCALAPPDATA%\CodexUsage`。`CODEX_USAGE_DATA_DIR` 可以显式更改位置；`PORT` 更改监听端口。网页资源从安装位置读取，不依赖当前目录。

旧源码版本的 `data/usage.sqlite` 不会自动移动。停止旧服务后，使用 `codex-usage migrate --from C:\absolute\old-checkout\data\usage.sqlite` 显式复制；工具拒绝覆盖已有目标，校验完整性和 SHA-256，并保留原件。

升级前运行 `codex-usage stop`。使用安装脚本升级时，会保留缓存并更新已启用的启动器；随后运行 `codex-usage skill install` 更新 Skill。手动 npm 升级后，对已启用自启动的安装重新运行 `autostart enable` 更新绝对路径。

卸载前执行 `autostart disable`、`stop` 和 `skill uninstall`，再按实际安装前缀执行 `npm uninstall --global codex-detailed-usage`。脚本安装前缀为 `%LOCALAPPDATA%\CodexUsage\tools`，卸载时加相应 `--prefix`。默认保留数据、日志、启动器文件和 Skill 备份，完整清理见 [Agent 安装文档](docs/INSTALL_FOR_AGENTS.md)。
...
打开 [工作台](http://127.0.0.1:8765)、[API 文档](http://127.0.0.1:8765/docs)或 [OpenAPI](http://127.0.0.1:8765/openapi.json)。`npm start` 是前台服务入口，可用 Ctrl+C 停止；产品入口是 `node bin/codex-usage.mjs`。运行 `npm run package:smoke` 可验证实际打包安装、隔离自启动和 CLI 生命周期。
...
界面使用 Lucide SVG 图标和本地打包的 Inter 字体。
...
- `%LOCALAPPDATA%\CodexUsage\usage.sqlite` 保存统计元数据、任务标题和账户快照；不保存聊天正文、工具输出或登录凭据。数据库包含项目路径和账户标识，发布源码和问题反馈时不要携带个人缓存。
...
`active` 是导入时确定的明细参与标记，不是汇总缓存。重复文件各自保留来源贡献，查询只统计选中的一份；文件重新解析后在事务内重新确定参与记录。删除过的源文件已导入数据会保留；如需彻底从现存文件重建，可停止服务后备份并移走本工具的缓存数据库再启动。
...
运行 `codex-usage skill install` 安装仓库内的 Skill。它通过正式 CLI 查询，命令从任何目录可用；已有非本工具管理的同名 Skill 会报告冲突。只有显式安装命令会改动 Skill 目录。
...
Skill 通过 CLI 调用 API，不直接读取会话或数据库；回答必须保留范围、来源、更新时间和缺失说明。`skills/codex-usage/scripts/query.mjs` 保留为兼容转发入口。`CODEX_USAGE_URL` 可显式连接另一个已运行的 HTTP loopback 服务；设置该变量时，查询和 status 不自动启动服务。查询 JSON 额外保留 `query`，标明实际使用的时间边界和筛选。
...
时间、维度、搜索、选择与分页保存到 URL；支持单日／小时下钻、Session 内轮次及跨任务 Turn 排行。
...
原创代码采用 [Apache-2.0](LICENSE)，依赖和字体许可见 [第三方声明](THIRD_PARTY_NOTICES.md)。贡献说明见 [CONTRIBUTING.md](CONTRIBUTING.md)。问题反馈请使用 [GitHub Issues](https://github.com/Cusnd/codex-usage/issues)，不要附带登录信息、真实会话、数据库或未脱敏截图。
