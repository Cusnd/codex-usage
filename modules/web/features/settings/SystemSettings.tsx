import { useApi, useCapabilities } from '../../runtime/context.js';
  ...
  return <section className="panel settings-panel startup-panel">
    ...
    <div className="startup-row">
      <div className="startup-copy">
        ...
        <p id="startup-description">{demoPreview ? '示例站无法管理设备启动项；安装本地应用后可用。' : '安静地启动本地服务，不弹出浏览器。关闭网页后仍可随时访问。'}</p>
      </div>
      <div className="startup-control">
        <span className="startup-state" aria-live="polite">{demoPreview ? '本地应用专属' : busy ? '更新中…' : query.isPending ? '读取中…' : query.error ? '读取失败' : !state ? '状态未知' : !state.supported ? '当前系统不支持' : state.conflict ? '启动项冲突' : state.enabled ? '已开启' : '已关闭'}</span>
    ...
    <div className="access-methods">
      <div className="access-method">
        ...
        {demoPreview ? <span className="access-domain">usage.esoren.com</span> : <a className="access-domain" href="https://usage.esoren.com" rel="noreferrer">usage.esoren.com<ArrowUpRight size={17} aria-hidden="true" /></a>}
        <p>{demoPreview ? '安装本地应用后使用；示例站不会连接你的本机服务。' : '服务启动后，输入域名即可打开本机工作台。'}</p>
      </div>
      <div className="access-method">
