import {ProjectIcon} from '../../widgets/ProjectIdentity.js';
import {useState} from 'react';
import {useQuery,useQueryClient} from '@tanstack/react-query';
import type { SyncCut } from '../../../contracts/sync.js';
import type {LogicalProject,ProjectEdge,ProjectDisplay} from '../../../organization/projects.js';
...
type View={cut:SyncCut;projects:(LogicalProject&{display?:ProjectDisplay})[];sources:Source[];aliases:Record<string,string>;blocked_edges:ProjectEdge[]};
...
export function CloudProjects(){
  ...
  const label=(p:LogicalProject&{display?:ProjectDisplay})=>p.display?.name||p.name||sources.get(p.members[0])?.name||sources.get(p.members[0])?.root||'未命名项目';
