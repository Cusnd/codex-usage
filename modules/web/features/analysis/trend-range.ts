...
import { bucketRange, type TrendBucket } from '../../../foundation/time-range.js';

export function readTrendParent(raw: string | null): { range: string; from: string; to: string; bucket: TrendBucket } | null {
  try {
    const value = JSON.parse(raw || 'null');
    if (!value || typeof value.from !== 'string' || typeof value.to !== 'string') return null;
    if (!['custom', 'today', '7', '30', 'all'].includes(value.range) || !['day', 'hour', 'week', 'month'].includes(value.bucket)) return null;
...
export function selectedBrushIndices(times: string[], bucket: TrendBucket, timezone: string, from: string, to: string) {
