...
export function useCloudDevices(){const sync=useCloud();return useQuery({queryKey:['cloud-devices'],queryFn:({signal})=>cloudRequest<{devices:CloudSource[]}>('/api/v3/devices','GET',undefined,signal),enabled:sync?.online!==false,refetchInterval:sync?.online===false?false:15000});}
