...
export async function changes(db:D1Database,user:string,epoch:string,after:number,limit=20,leaseId?:string):Promise<ChangesPage> {
  ...
  const lease=leaseId?await getRead(db,user,leaseId,'metadata'):null;if(lease&&lease.epoch!==epoch)fail(409,'BASELINE_REQUIRED','读取版本已变化。');const target=lease?.cut??h.commit_seq;if(after>target)fail(400,'INVALID_CURSOR','变化游标超出固定版本。');
