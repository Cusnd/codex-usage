...
export async function cloudRequest<T>(path:string,method='GET',body?:unknown,signal?:AbortSignal):Promise<T>{
  const r=await fetch(path,{method,signal,headers:body===undefined?{}:{'Content-Type':'application/json'},body:body===undefined?undefined:JSON.stringify(body)});
  const data=await r.json();if(!r.ok)throw Object.assign(new Error(data.error?.message||'云端请求失败。'),{status:r.status});return data;
}
export function CloudGate({children}:{children:ReactNode}){
  ...
  const me=useQuery({queryKey:['cloud-user'],queryFn:({signal})=>cloudRequest<{user:CloudUser}>('/api/v1/me','GET',undefined,signal),networkMode:'always',retry:false,refetchOnWindowFocus:true});
  ...
  return <QueryClientProvider client={sessionClient}><CloudSyncProvider userId={user.id}><CloudIdentity user={user} offline={!!me.error} onLogout={()=>{sessionClient.clear();client.clear();}}/>{children}</CloudSyncProvider></QueryClientProvider>;
}
...
export function CloudSyncStatus(){
  ...
  const labels={empty:'等待首次同步',legacy_ready:'完整旧版历史可读 · 正在迁移',recent_loading:'正在准备近期历史',recent_ready:'近期历史缓存已就绪',full_loading:'正在补齐完整历史',full_ready:'完整历史缓存已就绪',delta_loading:'正在接收最新变化'};
  const done=Object.values(state.baseline?.counts||{}).reduce((n,v)=>n+(v||0),0),total=state.baseline?.lease.total_entities;
  return <section className="cloud-sync-progress" aria-label="浏览器同步状态"><div><strong>{online?labels[state.phase]:'离线 · 查看已保存的页面'}</strong><span>{state.phase==='legacy_ready'?'正在迁移完整历史，暂时显示最近完整同步的旧版历史，完成后自动切换。':state.lastSyncAt?'最近完成 '+time(state.lastSyncAt,timezone):'首次打开将先准备近期记录，再继续补齐历史。'}</span></div>
    ...
    <p className="footnote">{state.phase==='legacy_ready'?'迁移期间需保持联网；旧版历史不标记为新版缓存完成。':'离线时可查看已打开并保存的页面及筛选结果。'}</p>{state.error&&<p className="error-text" role="alert">{state.error}</p>}
  </section>;
}
...
export function sourceState(d:CloudSource){return d.historyDeleting?'正在删除云端历史 · 设备已撤销':d.deletionStatus==='failed'?'删除未完成 · 设备已撤销':d.historyDeleted?'历史已删除 · 设备已撤销':d.revoked?'已撤销 · 历史保留':d.paused?'同步已暂停':d.protocol===1?'仅额度 · 可升级同步':d.error?'同步失败':!d.receivedAt?'等待首次同步':Date.now()-Date.parse(d.receivedAt)>180000?'设备离线或未更新':!d.initialComplete?'历史同步中':'最近已同步';}
...
export function CloudDeviceSettings(){
  ...
  return <section className="panel cloud-devices"><div className="panel-heading"><h2>采集设备</h2><Link to="/bind"><Plus size={16}/>绑定设备</Link></div>
    ...
    {(q.data?.devices||[]).map(d=><article key={d.id} className="cloud-device-row"><div><h3>{d.name}</h3><p>{sourceState(d)}</p><small>采集：{time(d.collectedAt,timezone)} · 接收：{time(d.receivedAt,timezone)}{d.protocol>=3&&<> · 应用：{time(d.appliedAt,timezone)}</>}</small><small>历史覆盖：{time(d.coverageFrom,timezone)} — {time(d.coverageTo,timezone)}</small><small>{d.protocol>=3?<>{d.retainedThreads===undefined?'云端保留会话数暂不可用':`云端已保留 ${d.retainedThreads} 个会话`} · 本次采集报告 {d.totalThreads??'未知'} 个会话</>:<>已同步 {d.syncedThreads} / {d.totalThreads??'未知'} 个会话</>}</small>{d.sourceCounts&&<small>云端完整来源 {d.sourceCounts.complete} / {d.sourceCounts.known} · 待应用批次 {d.sourceCounts.pendingBatches}{d.sourceCounts.unavailable>0?` · 暂不可用来源 ${d.sourceCounts.unavailable}`:''}</small>}</div>
...
export function CloudBind(){
  ...
  async function action(approve=false){setBusy(true);setError('');try{if(approve){await cloudRequest('/api/v2/device-authorizations/approve','POST',{code,replaceDeviceId:resume||null});setDone(true);}else{setResume('');setDevice(await cloudRequest('/api/v2/device-authorizations/inspect','POST',{code}));}}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
...
export function CloudAccounts({history=false}:{history?:boolean}){
  ...
  return <div className={history?'cloud-account-history':''}><ErrorBox error={q.error}/>{!q.error&&!q.data?.data.length&&<p className="notice">{q.isPending?'正在读取账户快照…':'暂无已确认的账户快照；请等待采集设备同步。'}</p>}
    {q.data?.data.map((a,i)=>{const historyDevices=new Set([a.historyMeta?.summarySource.deviceId,...(a.historyMeta?.dailySources.map(s=>s.deviceId)||[])].filter(Boolean));return <section key={a.accountRef} className={history?'panel account-chart':'cloud-account-group'}><h3>{a.accountRef.startsWith('legacy:')?'旧版设备快照':a.quota.accountRef?`账户 ${i+1}`:'身份尚未确认'} · {history?(historyDevices.size>1?`${historyDevices.size} 台设备的历史`:a.historyMeta?.summarySource.deviceName||'历史来源待确认'):a.deviceName}</h3>
      ...
      {a.accountRef.startsWith('legacy:')&&<p className="footnote">仅额度协议尚不能跨设备确认同一账户；升级该采集端后可统一识别。</p>}
