...
import { stableJson, type SyncEntity, type SyncCut } from '../shared/sync-v3';
import { USAGE_QUERY_REVISION } from '../shared/query-revision';
import { cloudMode, exampleMode } from './runtime';
import { CloudSyncController, jsonRequest, SyncError } from './cloud-sync/controller';
...
const example: UsageDataSource | undefined = import.meta.env?.MODE === 'showcase' ? {
  mode: 'example', revision: () => 'example', capture: () => undefined,
  async query<T>(route: string, params = {}, signal?: AbortSignal) {
    signal?.throwIfAborted(); const adapter = await (await import('../showcase/browser')).exampleAdapter(); signal?.throwIfAborted();
    return await adapter.request(route, params) as ApiResponse<T>;
  },
  async mutate<T>(route: string, body: unknown, method = 'POST') {
    return await (await (await import('../showcase/browser')).exampleAdapter()).request(route, {}, method, body) as ApiResponse<T>;
  },
} : undefined;
...
export class CloudUsageDataSource implements UsageDataSource {
  ...
  invalidateAccounts() {
    ...
    this.accountWrites.catch(() => {}); notifySource();
  }
...
let activeCloud: CloudUsageDataSource | undefined;
const subscribers = new Set<() => void>();
export function notifySource() { for (const listener of subscribers) listener(); }
export function installCloudDataSource(source: CloudUsageDataSource) {
  activeCloud = source; const unsubscribe = source.controller.subscribe(notifySource); notifySource();
  return () => { unsubscribe(); if (activeCloud === source) { activeCloud = undefined; notifySource(); } };
}
export const subscribeDataSource = (listener: () => void) => { subscribers.add(listener); return () => { subscribers.delete(listener); }; };
export const dataSourceRevision = () => cloudMode ? activeCloud?.revision() ?? 'cloud:initializing' : exampleMode ? 'example' : 'local';
export const cloudProjectName = (id: string) => activeCloud?.projectName(id);
export function usageDataSource(): UsageDataSource {
  if (exampleMode) return example!;
  if (cloudMode) { if (!activeCloud) throw new SyncError('Cloud data source is initializing.', 'SOURCE_NOT_READY'); return activeCloud; }
  return local;
}
