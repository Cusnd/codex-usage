...
export class CloudSyncController {
  ...
  initialize() {
    return this.initialized ??= this.cache.state(this.namespace).then(state => { this.current = state ?? emptyState(this.namespace); this.emit(); });
  }
  private persist(transform: (state: CacheState) => CacheWrite) {
  ...
  private async request<T>(operation: () => Promise<T>) {
    const generation = this.generation, value = await operation();
  ...
  private async beginBaseline(scope: 'recent' | 'full', fromLease?: string) {
    ...
    await this.persist(state => ({ state: { ...state, phase: scope === 'recent' ? 'recent_loading' : 'full_loading', error: null, deltaLease: null,
      baseline: { lease, cursor: null, started: false, counts: {}, last_key: null, page: null } } }));
  }
  private async freshLease(lease: ReadLease) {
    ...
    await this.persist(state => ({ state: { ...state,
      ...
      ...(state.activeLease?.lease_id === lease.lease_id ? { activeLease: next } : {}) } }));
    return next;
  }
  async view(signal?: AbortSignal): Promise<ReadLease> {
  ...
  private async baseline() {
    for (;;) {
      ...
      if (!progress.page) {
        const page = await this.request(() => this.transport.manifest(lease.lease_id, progress.cursor, this.abort.signal));
        ...
        await this.persist(state => ({ state: { ...state, baseline: { ...state.baseline!, lease: nextLease, started: true, counts,
      ...
      for (let offset = 0; offset < missing.length; offset += 100) {
        ...
        const wanted = missing.slice(offset, offset + 100), response = await this.request(() => this.transport.entities(lease.lease_id, wanted, this.abort.signal));
        ...
        await this.persist(state => ({ state: { ...state, baseline: { ...state.baseline!, lease: response.expires_at ? { ...state.baseline!.lease, expires_at: response.expires_at } : state.baseline!.lease } },
      ...
      if (finished.page!.next_cursor !== null) {
        await this.persist(state => ({ state: { ...state, baseline: { ...state.baseline!, cursor: finished.page!.next_cursor, page: null } } }));
        continue;
      }
      ...
      await this.persist(state => ({ state: { ...state, phase: lease.scope === 'recent' ? 'recent_ready' : 'full_ready', baseline: null,
  ...
  private async applyStagedCommits(lease: ReadLease) {
    ...
    for (const seq of sequences) {
      ...
      await this.persist(state => ({ state: { ...state, appliedCommitSeq: seq }, entities, deleted }));
    }
  }
  private async delta() {
    ...
    if (!lease) {
      ...
      await this.persist(state => ({ state: { ...state, deltaLease: lease, phase: 'delta_loading', error: null } }));
    }
    await this.applyStagedCommits(lease);
    while (this.current.appliedCommitSeq < lease.cut.commit_seq) {
      lease = await this.freshLease(lease);
      const before = this.current.appliedCommitSeq, currentLease = lease, page = await this.request(() => this.transport.changes(currentLease, before, this.abort.signal));
      ...
      await this.persist(state => ({ state: { ...state, receivedCommitSeq: received }, stages: staged }));
      await this.applyStagedCommits(lease);
    ...
    await this.persist(state => ({ state: { ...state, activeLease: lease, deltaLease: null, phase: 'full_ready', receivedCommitSeq: lease.cut.commit_seq,
