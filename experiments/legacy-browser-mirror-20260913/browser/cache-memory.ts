import { stableJson, type SyncEntity } from '../../../modules/contracts/sync.js';
import { type CloudCache, type CacheState, type AccountState, type StagedRow, type CachedQuery, type CachedAccountQuery, clone, type EntityRef, entityId, type ManifestEntry, stagePrimary, type StagedEntity, sameCut, queryLeaseOf, type AccountCacheRead, accountNamespace, type CacheWrite, CacheConflict, sameUser } from './cache-model.js';
...
export class MemoryCloudCache implements CloudCache {
  ...
  async putQuery(namespace: string, query: CachedQuery) {
    if (sameCut(queryLeaseOf(this.states.get(namespace))?.cut, query.cut)) this.queries.set(stableJson([namespace, query.key]), { namespace, query: clone(query) });
  }
