import { type SnapshotManifestPage } from '../../../modules/contracts/sync.js';
import { CacheConflict, emptyState, entityKey, namespaceOf, sameCut, queryLeaseOf, type CacheIdentity, type CacheState, type CacheWrite, type CloudCache, type ReadLease, type ManifestEntry, type StagedCommit } from "./cache.js";
import { type SyncTransport, HttpSyncTransport, SyncError } from './transport.js';
import { assert, validCut, verifyLease, entryValid, compareRef, verifyEntities, integer, kinds } from './verify.js';
...
export class CloudSyncController {
  ...
  private renewals = new Map<string, Promise<ReadLease>>();
  private recoveries = new Map<string, Promise<ReadLease>>();
  private invalidLeases = new Set<string>();
  ...
  state = () => this.current;
  queryLease = () => queryLeaseOf(this.current);
  ...
  private async cycle(reason: Trigger) {
    ...
    const active = this.queryLease()?.cut;
    ...
    try {
      ...
      if (error instanceof SyncError && error.code === 'BASELINE_REQUIRED') {
        const oldRun = this.current.baseline?.lease.lease_id ?? this.current.deltaLease?.lease_id, previousCut = this.queryLease()?.cut;
        ...
        await this.beginBaseline(this.current.activeLease ? 'full' : 'recent', undefined, previousCut); await this.baseline();
  ...
  private async beginBaseline(scope: 'recent' | 'full', fromLease?: string, priorQueryCut = this.queryLease()?.cut) {
    ...
    if (priorQueryCut && lease.cut.deletion_version > priorQueryCut.deletion_version) {
      await this.invalidateForDeletion(); generation = this.generation;
    }
    await this.persist(state => ({ state: { ...state, phase: scope === 'recent' ? 'recent_loading' : 'full_loading', error: null, deltaLease: null,
      viewAt: !state.activeLease && !sameCut(queryLeaseOf(state)?.cut, lease.cut) ? new Date(this.now()).toISOString() : state.viewAt,
  ...
  private async freshLease(lease: ReadLease) {
    ...
    const saved = [this.current.activeLease, this.current.baseline?.lease, this.current.deltaLease]
      .find(value => value?.lease_id === lease.lease_id && sameCut(value.cut, lease.cut));
    if (saved && Date.parse(saved.expires_at) > Date.parse(lease.expires_at)) lease = saved;
    const key = JSON.stringify([generation, lease.lease_id, lease.cut]);
    if (this.invalidLeases.has(key)) throw new SyncError('This read lease requires recovery.', 'BASELINE_REQUIRED', 409);
    if (Date.parse(lease.expires_at) > this.now() + 60_000) return lease;
    const existing = this.renewals.get(key); if (existing) return existing;
    const operation = (async () => {
      const renewed = await this.request(() => this.transport.renew(lease.lease_id, this.abort.signal), generation);
      ...
      if (error instanceof SyncError && error.code === 'BASELINE_REQUIRED') this.invalidLeases.add(key);
      throw error;
    }).finally(() => { this.renewals.delete(key); });
    this.renewals.set(key, operation); return operation;
  }
  async view(signal?: AbortSignal): Promise<ReadLease | null> {
    ...
    if (!this.queryLease()) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const ready = () => { if (this.queryLease()) { cleanup(); resolve(); } };
        const unsubscribe = this.subscribe(ready);
        const cancelled = () => { cleanup(); reject(signal?.reason); };
        signal?.addEventListener('abort', cancelled, { once: true });
        ready(); work.then(() => { if (!this.queryLease()) { cleanup(); reject(new SyncError('No verified cloud read is available.')); } }, error => { cleanup(); reject(error); });
      });
    }
    const lease = this.queryLease(); assert(lease, 'No verified cloud read is available.');
    return lease;
  }
  async renewView(lease: ReadLease) {
    const current = this.queryLease();
    return this.freshLease(current && sameCut(current.cut, lease.cut) ? current : lease);
  }
  /** Recover one invalid online read; callers must reject a changed cut before displaying it. */
  async recoverView(lease: ReadLease) {
    await this.initialize();
    const generation = this.generation, key = JSON.stringify([generation, lease.lease_id, lease.cut]);
    this.invalidLeases.add(key);
    const current = this.queryLease();
    if (current && current.lease_id !== lease.lease_id) return current;
    const existing = this.recoveries.get(key); if (existing) return existing;
    const replacement = (work: Promise<void>) => new Promise<ReadLease | null>((resolve, reject) => {
      let finished = false;
      const complete = (lease: ReadLease | null, error?: unknown) => {
        if (finished) return; finished = true; unsubscribe();
        if (error !== undefined) reject(error); else resolve(lease);
      };
      const changed = () => { const next = this.queryLease(); if (next && next.lease_id !== lease.lease_id) complete(next); };
      const unsubscribe = this.subscribe(changed);
      work.then(() => complete(this.queryLease()), error => complete(null, error)); changed();
    });
    const operation = (async () => {
      if (this.running) await replacement(this.running).catch(() => {});
      this.assertGeneration(generation);
      if (!this.queryLease() || this.queryLease()!.lease_id === lease.lease_id) await replacement(this.trigger('initial'));
      this.assertGeneration(generation);
      const next = this.queryLease(); assert(next, 'No verified cloud read is available after recovery.');
      return next;
    })().finally(() => { this.recoveries.delete(key); });
    this.recoveries.set(key, operation); return operation;
  }
  private async baseline() {
    ...
    for (;;) {
      ...
      for (let offset = 0; offset < missing.length; offset += 200) {
        ...
        // At most two 100-entity requests overlap. Validation and durable writes
        // retain their manifest order, and a failed request drains its sibling.
        const batches = [missing.slice(offset, offset + 100), missing.slice(offset + 100, offset + 200)].filter(batch => batch.length);
        const requests = batches.map(wanted => this.request(() => this.transport.entities(lease.lease_id, wanted, localAbort.signal), generation));
        let responses: Awaited<(typeof requests)[number]>[];
        try { responses = await Promise.all(requests); }
        catch (error) { localAbort.abort(error); await Promise.allSettled(requests); throw error; }
        for (const [index, response] of responses.entries()) {
          const wanted = batches[index];
      ...
      await persist(state => ({ state: { ...state, phase: lease.scope === 'recent' ? 'recent_ready' : 'full_ready', baseline: null,
        ...
        viewAt: sameCut(queryLeaseOf(state)?.cut, lease.cut) ? state.viewAt : new Date(this.now()).toISOString(), error: null },
...
export { SyncError, jsonRequest, HttpSyncTransport, type SyncTransport, type ChangeResponse } from './transport.js';
export { entityHash } from './verify.js';
