...
import { type CloudCache, request, type CacheState, type EntityRef, entityId, type StoredEntity, stageRun, type StoredStage, type ManifestEntry, stagePrimary, type StagedEntity, type StoredQuery, type CachedQuery, done, sameCut, queryLeaseOf, type AccountCacheRead, accountNamespace, type AccountState, type StoredAccountQuery, type CachedAccountQuery, type CacheWrite, CacheConflict, cursorEach, sameUser, forEachBatch, tupleRange } from './cache-model.js';
...
export class IndexedDbCloudCache implements CloudCache {
  ...
  async putQuery(namespace: string, query: CachedQuery) {
    ...
    if (sameCut(queryLeaseOf(state)?.cut, query.cut)) tx.objectStore('query_results').put({ key: stableJson([namespace, query.key]), namespace, query });
    await complete;
  }
