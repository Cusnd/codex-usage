import { IndexedDbCloudCache, emptyState, namespaceOf, CacheConflict, type ReadLease, type CacheState } from '../browser/cache.js';
import { entityHash } from '../browser/controller.js';
...
await run('cold query results persist before entity completion and cannot survive a cross-tab deletion', async () => {
  const own = namespaceOf({ origin: location.origin, userId: 'cold-test-user', deviceIds: ['one'] });
  const initial = emptyState(own), coldLease = { ...lease, lease_id: 'cold-query', scope: 'recent' as const };
  let cold = await cache.write(initial, { state: { ...initial, phase: 'recent_loading', baseline: { lease: coldLease, cursor: null, started: false, counts: {}, last_key: null, page: null } } });
  const query = { key: 'cold-page', cut, response: { data: { totalTokens: '9007199254740993' }, meta: { source: 'cloud', updatedAt: null, timezone: 'UTC', warnings: [] } }, cachedAt: new Date().toISOString() };
  await cache.putQuery(own, query);
  const other = new IndexedDbCloudCache(indexedDB, name);
  check((await other.query(own, query.key))?.response.data !== undefined, 'cold server result was not durable');
  const saved = (await other.state(own))!;
  check(saved.activeLease === null && saved.coverage === null && saved.phase === 'recent_loading', 'query write falsely promoted entity coverage');
  await other.putQuery(own, { ...query, key: 'wrong-cut', cut: { ...cut, commit_seq: 9 } });
  check(!await cache.query(own, 'wrong-cut'), 'unverified cut was cached');
  const deletion = cache.write(cold, { state: emptyState(own), resetUser: true });
  const late = other.putQuery(own, query); cold = await deletion; await late;
  check(!await cache.query(own, query.key) && cold.baseline === null, 'late cold query undid deletion');
  other.close();
});
await run('an existing complete cut remains the only query cache target during a newer baseline', async () => {
  const own = namespaceOf({ origin: location.origin, userId: 'cold-priority-user', deviceIds: [] });
  const initial = emptyState(own), nextLease = { ...lease, lease_id: 'later', cut: { ...cut, commit_seq: 2 } };
  await cache.write(initial, { state: { ...initial, phase: 'full_loading', activeLease: lease, baseline: { lease: nextLease, cursor: null, started: false, counts: {}, last_key: null, page: null } } });
  const query = { key: 'priority', cut, response: { data: { old: true }, meta: { source: 'cloud', updatedAt: null, timezone: 'UTC', warnings: [] } }, cachedAt: new Date().toISOString() };
  await cache.putQuery(own, query); await cache.putQuery(own, { ...query, cut: nextLease.cut, response: { ...query.response, data: { old: false } } });
  check((await cache.query(own, query.key))?.cut.commit_seq === 1, 'new incomplete baseline replaced the captured complete view');
});
