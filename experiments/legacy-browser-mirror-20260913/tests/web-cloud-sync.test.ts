...
import { CloudSyncController, entityHash, SyncError, type SyncTransport, type ChangeResponse } from '../browser/controller.js';
import { MemoryCloudCache, emptyState, namespaceOf, CacheConflict, type CacheWrite, type CacheState, type ReadLease, type ManifestEntry } from '../browser/cache.js';
import { CloudUsageDataSource, OfflineCacheMiss } from '../browser/data-source.js';
import { namesFromProjects, referencedProjectIds } from '../browser/project-labels.js';
...
test('an expired page lease tries one bounded online recovery and stops if the server still rejects it', async () => {
  ...
  await assert.rejects(source.query('local/overview'), /稍后重试刷新/); assert.equal(transport.reads.length, reads + 1);
});
