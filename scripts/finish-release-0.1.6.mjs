// One-shot continuation of the user's explicitly authorized 0.1.6 release.
...
const branch = 'codex/release-0.1.6', tag = 'v0.1.6';
assert.equal(process.env.GITHUB_REF_NAME, branch);
assert.equal(JSON.parse(readFileSync('package.json')).version, '0.1.6');
...
const initial = '2ba0cd9cbc69553772a2c73f23c9e6993d413a00';
...
await api(`${base}/pulls/${pull.number}/merge`, 'PUT', { sha: head, merge_method: 'squash', commit_title: 'release: prepare 0.1.6' }).then(r => assert.equal(r.merged, true));
...
const release = await api(`${base}/pulls`, 'POST', {
  head: 'develop', base: 'main', title: 'Release 0.1.6: smooth chart transitions',
  body: `Fixes cold chart transitions, preserves animations across identical refetches, and prepares curve geometry once per target. Adds continuous day/hour morphing and moving tooltips with a 297ms nonlinear curve transition.\n\nAll 18 platform/runtime checks passed for the exact release tree: ${evidence}.`,
});
...
git('tag', '-a', tag, 'origin/main', '-m', 'Release 0.1.6');
git('push', 'origin', tag);
...
summary(`Published and registry-verified @esoren/codex-usage@0.1.6; main synchronized to develop. ${publication.html_url}`);
